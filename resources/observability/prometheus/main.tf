...
resource "kubernetes_manifest" "grafana_kube_apiserver" {
  manifest = yamldecode(templatefile("${path.module}/grafana/cm-kube-apiserver.yaml", {
   name      = "grafana-kube-apiserver"
   data      = file("${path.module}/grafana/kube-apiserver.json")
  }))
}